...
<content>
		...
		<script type="text/javascript">
			...
			tabs = [['server1', 'Server 1 <i class="icon-tools"></i>'],['server2', 'Server 2 <i class="icon-tools"></i>']];
				...
				<div class="section" id="htmlConf">
					<script type="text/javascript">
						...
						for (i = 0; i < tabs.length; ++i)
						{
							...
							htmlOut += '</div>';
							htmlOut += '<button type="button" value="go" onclick="toggle(\'vpn'+t+'\', vpn'+(i+1)+'up)" id="_vpn'+t+'_button" class="btn">' + (eval('vpn'+(i+1)+'up') ? '<i class="icon-stop"></i> Stop' : '<i class="icon-play"></i> Start ') + ' Now</button>';
							htmlOut += '</div>';
						}
